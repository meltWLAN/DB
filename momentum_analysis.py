...
class MomentumAnalyzer:
    ...
    def analyze_stocks(self, stock_list, sample_size=100, min_score=60, lookback_days=None, custom_weights=None, gui_callback=None):
        """分析股票列表，找出具有强劲动量的股票
        ...
        if gui_callback:
            gui_callback("progress", (f"准备分析 {original_count} 支股票", 5))
        
        # 限制样本大小
        if sample_size < len(stock_list):
            # 根据交易量和流通市值优先选择较活跃的股票，而不是完全随机
            if 'vol' in stock_list.columns:
                # 按最近交易量降序排序（如果有这个数据）
                stock_list = stock_list.sort_values('vol', ascending=False).head(sample_size * 2)
                # 从交易量较大的股票中随机选择，保持一定随机性
                stock_list = stock_list.sample(sample_size)
                logger.info(f"从 {original_count} 支股票中选择交易活跃度较高的 {sample_size} 支进行分析")
            else:
                stock_list = stock_list.sample(sample_size)
                logger.info(f"从 {original_count} 支股票中随机选择 {sample_size} 支进行分析")
            
            if gui_callback:
                gui_callback("progress", (f"从 {original_count} 支股票中选择 {sample_size} 支进行分析", 10))
        else:
        ...
        if gui_callback:
            gui_callback("progress", (f"开始分析 {total} 支股票", 15))
        
        # 使用自定义回溯天数或默认设置
        if lookback_days:
            self.lookback_period = lookback_days
        
        # 优化内存使用：根据股票数量自动调整批处理大小
        batch_size = 30 if total > 100 else total
        
        # 创建结果列表，使用共享变量来实时更新进度
        processed_count = 0
        valid_results = 0
        ...
        for batch_start in range(0, total, batch_size):
            ...
            current_batch = list(stock_list.iloc[batch_start:batch_end].iterrows())
            
            logger.info(f"处理批次 {batch_start//batch_size + 1}/{(total+batch_size-1)//batch_size}: {batch_start}-{batch_end-1}")
            
            # 使用串行处理
            for i, (idx, stock) in enumerate(current_batch):
                # 更新进度条
                processed_count += 1
                progress = 15 + int(85 * (processed_count / total))  # 从15%开始，最高到100%
                if gui_callback:
                    gui_callback("progress", (f"正在分析 [{processed_count}/{total}]: {stock['name']}({stock['ts_code']})", progress))
                
                result = self._analyze_single_stock((idx, stock))
                if result and result['total_score'] >= min_score:  # 使用最低分数阈值过滤
                    results.append(result)
                    valid_results += 1
                    if gui_callback:
                        gui_callback("progress", (f"已完成 [{processed_count}/{total}]: {stock['name']}({stock['ts_code']}) - 得分: {result['total_score']:.1f}", progress))
                else:
                    if gui_callback:
                        gui_callback("progress", (f"已跳过 [{processed_count}/{total}]: {stock['name']}({stock['ts_code']}) - 不符合条件", progress))
        ...
        if gui_callback:
            gui_callback("progress", (f"动量分析完成，共找到 {len(results)} 支符合条件的股票", 100))
