...
from tkinter.scrolledtext import ScrolledTex
import pandas as pd
import numpy as np
...
# 确保当前目录在Python路径中
current_dir = Path(__file__).paren
...
class StockAnalysisGUI:
    ...
    def setup_momentum_tab(self):
        ...
        # 最低得分
        ttk.Label(param_frame, text="最低得分:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        self.min_score_var = tk.StringVar(value="60")
        ttk.Entry(param_frame, textvariable=self.min_score_var, width=10).grid(row=1, column=1, padx=5, pady=5)
        # 分析按钮
    ...
    def setup_ma_cross_tab(self):
        ...
        # 样本大小
        ttk.Label(param_frame, text="样本数量:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
        self.ma_sample_size_var = tk.StringVar(value="20")
        ttk.Entry(param_frame, textvariable=self.ma_sample_size_var, width=10).grid(row=4, column=1, padx=5, pady=5)
        # 分析按钮
        ...
        try:
            ...
            def run_analysis():
                try:
                    ...
                    # 运行分析
                    self.status_bar["text"] = f"开始分析 {len(self.stock_list)} 支股票的动量..."
                    self.momentum_results = self.momentum_analyzer.analyze_stocks(
                        self.stock_list.head(sample_size), min_score=min_score)
                    # 更新结果
    ...
    def run_ma_cross_strategy(self):
        ...
        try:
            ...
            # 更新状态
            self.status_bar["text"] = "正在运行均线交叉策略..."
            # 在新线程中运行策略
            def run_strategy():
                try:
                    # 获取股票列表
                    ...
                    # 运行策略
                    self.status_bar["text"] = f"开始分析 {len(self.stock_list)} 支股票的均线交叉策略..."
                    self.ma_results = self.ma_strategy.run_strategy(
                        self.stock_list.head(sample_size),
                        short_ma=short_ma,
                        long_ma=long_ma,
